...
pub fn main() !void {
    ...
    //var gears: [input.len]i32 = undefined;
    var gears = std.mem.zeroes([input.len]i32);
    var sum: i64 = 0;
        ...
        for (line, 0..) |c, i| {
            if (std.ascii.isDigit(c)) {
                ...
                if (digit_count > 0 and found_symbol) {
                    ...
                    var other_number = gears[@intCast(gear_idx)];
                    if (other_number != 0) {
                        sum += number * @as(i32, @intCast(other_number));
                        //std.debug.print("engine part: {d}\n", .{ number });
                    } else {
                        gears[@intCast(gear_idx)] = @intCast(number);
                    }
                }
